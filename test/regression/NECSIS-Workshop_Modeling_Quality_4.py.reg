...
c0_Car.addConstraint(implies(some(join($this(), c0_total_mass)), equal(joinRef(join($this(), c0_total_mass)), sum(join(glob(c0_FuelFeature), c0_mass)))));
...
min(joinRef(join(glob(c0_aCar), c0_total_cost)));
min(joinRef(join(glob(c0_aCar), c0_total_fuel)));
max(joinRef(join(glob(c0_aCar), c0_total_comfort)));
min(joinRef(join(glob(c0_aCar), c0_total_mass)));
scope({c0_ComfortFeature:8, c0_Feature:10, c0_FuelFeature:5, c0_comfort:8, c0_cost:10, c0_fuel:5, c0_mass:10});
defaultScope(1);
stringLength(16);
