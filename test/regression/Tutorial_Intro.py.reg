...
c0_BusLine.addConstraint(set_in(glob(c0_JohnDoe), joinRef(join($this(), c0_participants))));
c0_BusLine.addConstraint(set_in(glob(c0_MaryJane), joinRef(join($this(), c0_participants))));
...
c0_JohnDoe2.addConstraint(equal(joinRef(join($this(), c1_Country)), glob(c0_Canada)));
